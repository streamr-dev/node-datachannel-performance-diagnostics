...
Peer::Peer(string _selfId, string _peerId, shared_ptr<WebSocket> _signalingServerWs, int _packetSize, bool _isSender):
	...
	this->logId = this->selfId + "->" + this->peerId;
        ...
        SctpSettings settings = {};
        settings.recvBufferSize = 64 * 1024 * 1024;
        settings.sendBufferSize = 64 * 1024 * 1024;
        SetSctpSettings(std::move(settings));

        Configuration config = {};
        this->connection = make_shared<PeerConnection>(std::move(config));
